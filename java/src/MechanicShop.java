...
//Added libraries
//INCLUE THE JAR FILE ADDED IN THE LIB FOLDER
...
public class MechanicShop{
	...
	public static void AddCar(MechanicShop esql){//3
		...
		try {
			...
			System.out.println("Link with a Customer (y/n)");
			...
			if (!input.equals("y")) {
				...
				temp = String.format("SELECT fname, lname FROM Customer WHERE lname = '%s';", input);
				List<List<String>> out = esql.executeQueryAndReturnResult(temp);
				
				if (out.size() == 0) {
					System.out.println("Error: Customer Not in DataBase");
					return;
				}
				...
				for (int i = 0; i < out.size(); ++i) {
					out_fname = out.get(i).get(0);
					out_lname = out.get(i).get(1);
					print = String.format("%d : %s %s", i, out_fname, out_lname);
				...
				while (val < 0 || val > out.size()) {
					...
					if (Integer.parseInt(temp) < 0 || Integer.parseInt(temp) > out.size() - 1) {
						System.out.println("Invalid #");
						temp = in.readLine();
						continue;
					}				
				...
				temp = String.format("SELECT id FROM Customer WHERE fname = '%s' AND lname = '%s';", fname, lname);
				out = esql.executeQueryAndReturnResult(temp);
				int cust_id = Integer.parseInt(out.get(0).get(0));
				temp = String.format("SELECT ownership_id FROM Owns ORDER BY ownership_id DESC LIMIT 1");
				out = esql.executeQueryAndReturnResult(temp);
				temp = out.get(0).get(0);
				int own_id = Integer.parseInt(temp) + 1;
				
				//insert own 
				temp = String.format("INSERT INTO Owns(ownership_id, customer_id, car_vin) VALUES(%d, %d, '%s');", own_id, cust_id, vin);
				esql.executeUpdate(temp);
